...
<footer>
  ...
  <span class="mono">OpenRouter proxy &middot; {{ 'live' if settings.openrouter_api_key else 'no api key set' }}</span>
</footer>
</div>
<script>
...
<section style="padding:90px 28px 60px;">
  <div class="container" style="max-width:840px;">
    ...
    <div style="display:flex;gap:14px;">
      ...
      {% else %}
        <a href="{{ url_for('register') }}" class="btn">Start generating &mdash; free</a>
      {% endif %}
...
<section class="container" style="max-width:420px;padding:70px 28px;">
  ...
  <p style="color:var(--muted);font-size:14px;margin-bottom:26px;">Free tier includes 8 image renders / 30 days.</p>
...
def register():
    if request.method == "POST":
        ...
        flash("Welcome to PRISM." + (" You're the first user, so you're the admin." if is_first else ""))
